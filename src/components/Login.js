...
const Login = () => {
  ...
  return (
    <div className="">
      ...
      <form className="w-3/12 absolute p-12 bg-black text-white my-36 mx-auto right-0 left-0 rounded-lg">
        <h1 className="font-bold text-3xl pb-4">{isSignInForm ? "Sign In" : "Sign Up"}</h1>

        {!isSignInForm && (<input type="text" placeholder="Full Name" className="p-3 my-3 w-full"/>)}

        <input type="text" placeholder="Email Address" className="p-3 my-3 w-full"/>
        <input type="password" placeholder="Password" className="p-3 my-3 w-full"/>
        <button className="p-3 w-full bg-red-700 my-3 rounded-lg ">{isSignInForm ? "Sign In" : "Sign Up"}</button>
